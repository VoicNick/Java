...
public class CustomerDao extends Dao {
	...
	public static final String TABLE_NAME = "A00884836_Customers";
	...
	public Customer getCustomer(String id) throws SQLException, ApplicationException {
		...
		try {
			...
			while (resultSet.next()) {
				...
				customer = new Customer.Builder(id_int, resultSet.getString(PHONE)).firstName(resultSet.getString(FIRSTNAME)).lastName(resultSet.getString(LASTNAME))
						.street(resultSet.getString(STREET)).city(resultSet.getString(CITY)).postalCode(resultSet.getString(POSTALCODE)).emailAddress(resultSet.getString(EMAIL))
						.joinDate(LocalDate.of(year, month - 1, day)).build();
			}
		} finally {
			close(statement);
		}
	...
	public List<Customer> readAllCustomers() throws SQLException, Exception {
		...
		try {
			...
			while (resultSet.next()) {
				...
				customer = new Customer.Builder(resultSet.getInt(ID), resultSet.getString(PHONE)).firstName(resultSet.getString(FIRSTNAME)).lastName(resultSet.getString(LASTNAME))
						.street(resultSet.getString(STREET)).city(resultSet.getString(CITY)).postalCode(resultSet.getString(POSTALCODE)).emailAddress(resultSet.getString(EMAIL))
						.joinDate(LocalDate.of(year, month - 1, day)).build();
